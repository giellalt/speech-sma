...
: ${OUTPUT_DIR:="./output_full_aj"}
: ${LOG_FILE:=$OUTPUT_DIR/nvlog.json}
: ${DATASET_PATH:=anna_jacobsen_full}
...
: ${TRAIN_FILELIST:=filelists/anna-jacobsen-full-short-splits-table-pitch-shuf.txt}
...
: ${VAL_FILELIST:=filelists/anna-jacobsen-full-short-splits-table-pitch-shuf-val.txt}
: ${AMP:=true} # voi laittaa true!
: ${SEED:=""}
...
python train_1_with_plot.py $ARGS "$@"
